...
- `VERIFYING`에서는 과거 이름을 확정 이름으로 표시하지 않는다. 30초 안의 재등장에는
  `이름?`를 표시한다. 다인 연결이 불확실한 경우에는 기존 `VERIFIED` 이름을 유지한 채
  화면에 표시하지 않는 백그라운드 재검증만 수행한다.
...
IoU를 비용으로 모든 Track과 현재 검출 상자의 전역 최적 배정을 계산한다. 이 배정은
Hungarian 알고리즘으로 계산하므로 다인 프레임에서도 조합 탐색으로 멈추지 않는다. 두 후보의
비용 차이가 작거나, 확정 Track 상자가 겹치거나, 이동 연속성이 깨지면
`TRACK_ASSOCIATION_UNCERTAIN` 이벤트를 낸다. 이 이벤트는 Track이나 현재 이름을 즉시 끊지 않고,
`IdentityReverificationService`에 고품질·비중복 얼굴 근거 수집만 예약한다. 현재 프로필을 지지하는
근거 두 개면 아무 화면 변화 없이 유지하고, 다른 프로필 또는 미등록 인물을 지지하는 근거 두 개가
누적될 때만 새 ObservationSession에서 신원을 다시 판정한다. 사람 추적기는 신원 판단을 수행하지
않으며 얼굴 임베딩 검증만이 이름 표시의 최종 근거다.
